            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <select class="form-control" name="package_id">
                    <?php foreach ($data_packet as $key=>$val): ?>
                        <option value="<?= $val['id']?>" <?php if($val['id'] == $package_id){ echo 'selected="selected"'; }?> ><?= $val['alias']; ?></option>
                    <?php endforeach;?>
                </select>
            </div>
        </form>
...
<table class="table table-hover">
    ...
    <tr>
        ...
        <th class="text-center">中否</th>
    </tr>

    <?= $this->render('_list',['model'=>$model,'type'=>$type, 'package_id' => $package_id])?>
...
<div class="text-center more" style="margin-bottom: 30px" data-page="2" data-url="<?= \yii\helpers\Url::to('/home/new-code')?>?package_id=<?= $package_id?>">
    <span style="padding: 9px 31px;" class="label label-success">加载更多</span>
</div>

<div class="text-center null" style="display: none;margin-bottom: 30px" data-page="2" data-url="<?= \yii\helpers\Url::to('/home/new-code')?>?package_id=<?= $package_id?>">
